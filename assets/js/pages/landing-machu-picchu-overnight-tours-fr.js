(function () {
  ...
  const LANDING_NAME = "Machu Picchu Overnight + Expériences au Pérou";
  ...
  const CALENDAR_OCCUPIED_LABEL = "Journée occupée";
  ...
  function getCircuitAvailability(circuit, dateStr) {
    ...
    if (circuit.enabled === false) return { available: false, reason: circuit.disabledReason || "Non disponible" };
  ...
  function validateDestinationTransition(selectedTours) {
    ...
    for (let i = 0; i < dated.length; i += 1) {
      for (let j = i + 1; j < dated.length; j += 1) {
        ...
        if (freeDays < minGap) {
          return { valid: false, message: "Pour voyager entre Lima/Ica et Cusco, prévoyez au moins une journée de transfert. Sélectionnez une date ultérieure pour continuer.", conflictingIds: [a.id, b.id] };
        }
      }
    }
  ...
  function validateAllDates(options) {
    ...
    selectedTours.forEach((t) => {
      if (!t.date) {
        ...
        valid = false;
        if (!silent) setFieldError(t.id, "Vous pouvez réserver ce séjour Overnight à partir de demain.");
      }
    });
  ...
  function renderCircuitOptions() {
    ...
    target.innerHTML = `
      ...
      ${availableCount ? "" : `<div class="mpt-circuit-soldout"><i class="fas fa-circle-xmark"></i><div><strong>Circuit 1 indisponible</strong><span>Choisissez un autre jour pour continuer.</span></div></div>`}
    `;
        ...
        <div class="mpt-main-product__grid">
          ...
          <div class="mpt-main-product__body">
            ...
            <section class="mpt-config-block" aria-labelledby="mptTrainTitle">
              ...
              <p class="mpt-train-upgrade-label"><i class="fas fa-arrow-up-right-dots" aria-hidden="true"></i> Options de surclassement pour le train aller et/ou retour</p>
              <div class="mpt-train-groups mpt-train-groups--upgrades">
                <fieldset class="mpt-choice-group">
                  <legend>Surclassement à l’aller</legend>
                  ${renderTrainOptions((p.trainSelection?.outbound || []).slice(1), state.mainProduct.outboundTrainId, "mptOutboundTrain", currency)}
                </fieldset>
                <fieldset class="mpt-choice-group">
                  <legend>Surclassement au retour</legend>
            ...
            <section class="mpt-config-block" aria-labelledby="mptMealTitle">
              <div class="mpt-config-block__head">
                <div><span class="mpt-step">4</span><div><strong id="mptMealTitle">Ajoutez une expérience gastronomique</strong><small>Le déjeuner est facultatif et calculé par voyageur payant.</small></div></div>
              </div>
  ...
  async function fetchLandingData() {
    const basePath = getSiteBasePath();
    const response = await fetch(`${basePath}assets/data/i18n/fr/landing-machu-picchu-overnight-tours.json`.replace(/([^:]\/)\/{2,}/g, "$1"), { cache: "no-store" });
