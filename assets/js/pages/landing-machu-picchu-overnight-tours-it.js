(function () {
  ...
  const LANDING_NAME = "Machu Picchu Overnight + Esperienze in Perù";
  ...
  const CALENDAR_OCCUPIED_LABEL = "Giorno occupato";
  ...
  function getCircuitAvailability(circuit, dateStr) {
    ...
    if (circuit.enabled === false) return { available: false, reason: circuit.disabledReason || "Non disponibile" };
  ...
  function validateDestinationTransition(selectedTours) {
    ...
    for (let i = 0; i < dated.length; i += 1) {
      for (let j = i + 1; j < dated.length; j += 1) {
        ...
        if (freeDays < minGap) {
          return { valid: false, message: "Per viaggiare tra Lima/Ica e Cusco è necessario almeno un giorno per il trasferimento. Seleziona una data successiva per continuare.", conflictingIds: [a.id, b.id] };
        }
      }
    }
  ...
  function validateAllDates(options) {
    ...
    selectedTours.forEach((t) => {
      if (!t.date) {
        ...
        valid = false;
        if (!silent) setFieldError(t.id, "Puoi prenotare questo Overnight a partire da domani.");
      }
    });
  ...
  function renderCircuitOptions() {
    ...
    target.innerHTML = `
      ...
      ${availableCount ? "" : `<div class="mpt-circuit-soldout"><i class="fas fa-circle-xmark"></i><div><strong>Circuito 1 non disponibile</strong><span>Scegli un altro giorno per continuare.</span></div></div>`}
    `;
        ...
        <div class="mpt-main-product__grid">
          ...
          <div class="mpt-main-product__body">
            ...
            <section class="mpt-config-block" aria-labelledby="mptTrainTitle">
              ...
              <p class="mpt-train-upgrade-label"><i class="fas fa-arrow-up-right-dots" aria-hidden="true"></i> Opzioni di upgrade per il treno di andata e/o ritorno</p>
              <div class="mpt-train-groups mpt-train-groups--upgrades">
                <fieldset class="mpt-choice-group">
                  <legend>Upgrade andata</legend>
                  ${renderTrainOptions((p.trainSelection?.outbound || []).slice(1), state.mainProduct.outboundTrainId, "mptOutboundTrain", currency)}
                </fieldset>
                <fieldset class="mpt-choice-group">
                  <legend>Upgrade ritorno</legend>
            ...
            <section class="mpt-config-block" aria-labelledby="mptMealTitle">
              <div class="mpt-config-block__head">
                <div><span class="mpt-step">4</span><div><strong id="mptMealTitle">Aggiungi un’esperienza gastronomica</strong><small>Il pranzo è facoltativo e viene calcolato per ogni viaggiatore pagante.</small></div></div>
              </div>
  ...
  async function fetchLandingData() {
    const basePath = getSiteBasePath();
    const response = await fetch(`${basePath}assets/data/i18n/it/landing-machu-picchu-overnight-tours.json`.replace(/([^:]\/)\/{2,}/g, "$1"), { cache: "no-store" });
