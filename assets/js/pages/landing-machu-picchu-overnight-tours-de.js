(function () {
  ...
  const LANDING_NAME = "Machu Picchu Overnight + Peru-Erlebnisse";
  ...
  function getCircuitAvailability(circuit, dateStr) {
    ...
    if (circuit.enabled === false) return { available: false, reason: circuit.disabledReason || "Nicht verfügbar" };
  ...
  function validateDestinationTransition(selectedTours) {
    ...
    for (let i = 0; i < dated.length; i += 1) {
      for (let j = i + 1; j < dated.length; j += 1) {
        ...
        if (freeDays < minGap) {
          return { valid: false, message: "Für die Reise zwischen Lima/Ica und Cusco benötigen Sie mindestens einen Transfertag. Wählen Sie ein späteres Datum, um fortzufahren.", conflictingIds: [a.id, b.id] };
        }
      }
    }
  ...
  function validateAllDates(options) {
    ...
    selectedTours.forEach((t) => {
      if (!t.date) {
        ...
        valid = false;
        if (!silent) setFieldError(t.id, "Sie können diese Overnight-Tour ab morgen buchen.");
      }
    });
  ...
  function renderCircuitOptions() {
    ...
    target.innerHTML = `
      ...
      ${availableCount ? "" : `<div class="mpt-circuit-soldout"><i class="fas fa-circle-xmark"></i><div><strong>Circuit 1 ist nicht verfügbar</strong><span>Wählen Sie einen anderen Tag, um fortzufahren.</span></div></div>`}
    `;
        ...
        <div class="mpt-main-product__grid">
          ...
          <div class="mpt-main-product__body">
            ...
            <section class="mpt-config-block" aria-labelledby="mptMealTitle">
              <div class="mpt-config-block__head">
                <div><span class="mpt-step">4</span><div><strong id="mptMealTitle">Fügen Sie ein kulinarisches Erlebnis hinzu</strong><small>Das Mittagessen ist optional und wird pro zahlendem Reisenden berechnet.</small></div></div>
              </div>
  ...
  async function fetchLandingData() {
    const basePath = getSiteBasePath();
    const response = await fetch(`${basePath}assets/data/i18n/de/landing-machu-picchu-overnight-tours.json`.replace(/([^:]\/)\/{2,}/g, "$1"), { cache: "no-store" });
