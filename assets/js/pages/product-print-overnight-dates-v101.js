...
(function () {
  ...
  function applyPatch() {
    ...
    if (proto.__mctPrintOnlyOvernightDatesV101Applied) return true;
      ...
      parsed.forEach(({ item, day, time }, index) => {
        ...
        if (normalizedDay !== lastDay) {
          ...
          rows.push(`<article class="print-itinerary-item">
            <div class="print-itinerary-time"><span class="print-time-main">${escapeHtml(ctx, dayLabel)}</span></div>
            <div><h3>${escapeHtml(ctx, formatDate(ctx, date))}</h3></div>
          </article>`);
    ...
    function addHotelNightDate(ctx, area, day1) {
      ...
      hotelSection.querySelectorAll(".print-hotel-night-date-v100, [data-mct-print-hotel-night-v101]").forEach((node) => node.remove());

      const row = document.createElement("small");
      row.setAttribute("data-mct-print-hotel-night-v101", "true");
    ...
    proto.applyPrintOnlyOvernightDatesV101 = function () {
      return applyPrintOnlyEnhancement(this);
    };

    proto.__mctPrintOnlyOvernightDatesV101Applied = true;
    return true;
  }
