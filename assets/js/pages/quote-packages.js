...
(function () {
  ...
  function renderPackageOptions() {
    ...
    const visibleCount = state.showAllItineraryOptions ? state.options.length : Math.min(4, state.options.length);
  ...
  function updatePrintableTemplate() {
    ...
    if (services) {
      ...
      services.innerHTML = `
        <div class="print-service-list print-service-list--clean">
          ...
          ${getOptionTours(option).map((tour) => `<p><strong>Tour:</strong> ${escapeHtml(tour.title || "Actividad incluida")}</p>`).join("")}
    ...
    if (hotelImages) {
      const hotelItems = Object.values(state.selectedHotels).filter((item) => item?.type === "hotel");
      hotelImages.innerHTML = hotelItems.map((item) => {
        const image = getHotelGalleryImages(item.hotel)[0];
        return image ? `
          <figure class="print-hotel-image-card">
            <img src="${escapeHtml(resolveAssetPath(image))}" alt="${escapeHtml(item.label || "Hotel")}">
            <figcaption>${escapeHtml(item.label || "Hotel seleccionado")} · ${escapeHtml(item.roomsSummary || "Habitación")}</figcaption>
          </figure>
        ` : "";
      }).join("");
    }
