(function () {
  ...
  const LANDING_NAME = "Machu Picchu + Tours au Pérou";
  ...
  function validateAllDates(options) {
    ...
    selectedTours.forEach((t) => {
      if (!t.date) {
        ...
        valid = false;
        if (!silent) setFieldError(t.id, "La date ne peut pas être antérieure à aujourd'hui.");
      }
    });
    ...
    Object.values(byDate).forEach((ids) => {
      if (ids.length > 1) {
        valid = false;
        if (!silent) ids.forEach((id) => setFieldError(id, "Deux tours ne peuvent pas avoir la même date."));
      }
    });
        ...
        <div class="mpt-main-product__grid">
          ...
          <div class="mpt-main-product__body">
            ...
            <div class="mpt-price-row">
              <div class="mpt-price-pill"><span>Adulte</span><strong>${formatCurrency(p.adultPrice, currency)}</strong></div>
              <div class="mpt-price-pill"><span>Enfant (${childPolicy.minAge}-${childPolicy.maxAge} ans)</span><strong>${formatCurrency(childPrice, currency)}</strong></div>
            </div>
            <div class="mpt-field-grid">
              ...
              <div class="mpt-field">
                <label for="mptAdults">Adultes</label>
                <select id="mptAdults" aria-label="Nombre d'adultes pour toute la réservation">${qtyOptions(1, 10, state.adults)}</select>
              </div>
              <div class="mpt-field">
                <label for="mptChildren">Enfants</label>
                <select id="mptChildren" aria-label="Nombre d'enfants pour toute la réservation">${qtyOptions(0, 10, state.children)}</select>
              </div>
            </div>
            <div>
              <strong style="font-size:0.85rem;">Inclus :</strong>
              <ul class="mpt-includes">${(p.includes || []).map((i) => `<li>${escapeHtml(i)}</li>`).join("")}</ul>
            </div>
            <div>
              <strong style="font-size:0.85rem;">Repas (facultatif) :</strong>
              <div class="mpt-meal-options" role="radiogroup" aria-label="Options de repas">
                ${(p.mealOptions || []).map((m) => `
                  <label class="mpt-meal-option">
                    <span>${escapeHtml(m.label)}${m.pricePerPerson > 0 ? ` (+${formatCurrency(m.pricePerPerson, currency)}/personne)` : " (inclus sans frais supplémentaires)"}</span>
  ...
  function renderSummary() {
    ...
    content.innerHTML = `
      ...
      <div class="mpt-summary__list">${linesHtml || '<p style="color:var(--mct-muted,#6c7a76); font-size:0.88rem;">Aucune expérience ajoutée pour le moment.</p>'}</div>
